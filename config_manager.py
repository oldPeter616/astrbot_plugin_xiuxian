...
class Config:
    ...
    def _load_all(self):
        ...
        # --- 预处理数据 ---
        self.level_map = {
            info["level_name"]: {"index": i, **info}
            for i, info in enumerate(self.level_data)
        ...
        self.realm_name_to_id = {
            info["name"]: realm_id
            for realm_id, info in self.realm_data.items()
            if "name" in info
        }
        self.boss_name_to_id = {
            info["name"]: boss_id
            for boss_id, info in self.boss_data.items()
            if "name" in info
        }
    ...
    def get_item_by_name(self, name: str) -> Optional[Tuple[str, Item]]:
        ...
        return (
            ...
            if item_id and item_id in self.item_data
            else None
        )
