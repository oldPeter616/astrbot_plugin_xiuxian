...
class SectHandler:
    ...
    async def handle_create_sect(
        self, event: AstrMessageEvent, sect_name: str, player: Player
    ):
        if not sect_name:
            yield event.plain_result(
                f"指令格式错误！请使用「{config.CMD_CREATE_SECT} <宗门名称>」。"
            )
            return

        success, msg, updated_player = await self.sect_manager.handle_create_sect(
            player, sect_name
        )
    ...
    async def handle_join_sect(
        ...
        if not sect_name:
            yield event.plain_result(
                f"指令格式错误！请使用「{config.CMD_JOIN_SECT} <宗门名称>」。"
            )
            return

        success, msg, updated_player = await self.sect_manager.handle_join_sect(
            player, sect_name
        )
        if success and updated_player:
            await self.db.update_player(updated_player)
    ...
    async def handle_my_sect(self, event: AstrMessageEvent, player: Player):
        ...
        if (
            leader_player
            and leader_player.user_id
            and leader_player.sect_id == sect_info["id"]
        ):
