...
def player_required(func):
    ...
    async def wrapper(self: 'XiuXianPlugin', event: AstrMessageEvent, *args, **kwargs):
        ...
        if not player:
            ...
            return
        # 将 player 对象作为第一个位置参数传递给原函数
        async for r in func(self, event, player, *args, **kwargs):
            yield r
    return wrapper
...
class XiuXianPlugin(Star):
    ...
    @player_required
    async def handle_buy(self, event: AstrMessageEvent, player: Player, item_name: str, quantity: int = 1):
    ...
    @player_required
    async def handle_use(self, event: AstrMessageEvent, player: Player, item_name: str, quantity: int = 1):
    ...
    @player_required
    async def handle_create_sect(self, event: AstrMessageEvent, player: Player, sect_name: str):
    ...
    @player_required
    async def handle_join_sect(self, event: AstrMessageEvent, player: Player, sect_name: str):
    ...
    @player_required
    async def handle_fight_boss(self, event: AstrMessageEvent, player: Player, boss_id: str):
