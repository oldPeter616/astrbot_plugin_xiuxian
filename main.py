...
class XiuXianPlugin(Star):
    ...
    async def _require_player(self, event: AstrMessageEvent) -> Optional[Player]:
        """
        检查玩家是否存在。如果不存在，则yield错误信息并返回None。
        如果存在，则返回Player对象。
        """
    ...
    async def handle_player_info(self, event: AstrMessageEvent):
        # 【修改】调用辅助方法
        async for player in self._require_player(event):
            if player:
    ...
    async def handle_realm_advance(self, event: AstrMessage-Event):
        async for player in self._require_player(event):
            if player:
