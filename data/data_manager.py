...
class DataBase:
    ...
    def __init__(self):
        self.data_dir: Path = StarTools.get_data_dir("xiuxian")
        self.db_path: Path = self.data_dir / config.DATABASE_FILE
        self._conn: aiosqlite.Connection = cast(aiosqlite.Connection, None)
    ...
    async def close(self):
        if self._conn:
            ...
            self._conn = cast(aiosqlite.Connection, None)
            logger.info("数据库连接已关闭。")

    # ---------------- Active World Boss ----------------
    async def get_active_bosses(self) -> List[ActiveWorldBoss]:
        async with self._conn.execute("SELECT * FROM active_world_bosses") as cursor:
            return [ActiveWorldBoss(**dict(row)) for row in await cursor.fetchall()]
    ...
    async def delete_active_boss(self, boss_id: str):
        await self._conn.execute(
            "DELETE FROM active_world_bosses WHERE boss_id = ?", (boss_id,)
        )
        await self._conn.commit()

    async def record_boss_damage(
        self, boss_id: str, user_id: str, user_name: str, damage: int
    ):
        await self._conn.execute(
            """
            ...
            SET total_damage = total_damage + excluded.total_damage;
            """,
    ...
    async def get_boss_participants(self, boss_id: str) -> List[Dict[str, Any]]:
        ...
        async with self._conn.execute(sql, (boss_id,)) as cursor:
    ...
    async def clear_boss_data(self, boss_id: str):
        try:
            ...
            await self._conn.execute(
                "DELETE FROM world_boss_participants WHERE boss_id = ?", (boss_id,)
            )
            await self._conn.commit()
    ...
    async def get_top_players(self, limit: int) -> List[Player]:
        async with self._conn.execute(
            "SELECT * FROM players ORDER BY level_index DESC, experience DESC LIMIT ?",
            (limit,),
        ) as cursor:
            return [Player(**dict(row)) for row in await cursor.fetchall()]
    ...
    async def create_player(self, player: Player):
        ...
        sql = f"INSERT INTO players ({', '.join(player_fields)}) VALUES ({', '.join([f':{f}' for f in player_fields])})"
        await self._conn.execute(sql, player.__dict__)
        await self._conn.commit()
    ...
    async def update_players_in_transaction(self, players: List[Player]):
        if not players:
            return
        player_fields = [f.name for f in fields(Player) if f.name != "user_id"]
        sql = f"UPDATE players SET {', '.join([f'{f} = :{f}' for f in player_fields])} WHERE user_id = :user_id"
        try:
            await self._conn.execute("BEGIN")
    ...
    # ---------------- Sects ----------------
    async def create_sect(self, sect_name: str, leader_id: str) -> int | None:
        async with self._conn.execute(
            "INSERT INTO sects (name, leader_id) VALUES (?, ?)", (sect_name, leader_id)
        ) as cursor:
            await self._conn.commit()
            return cursor.lastrowid

    async def delete_sect(self, sect_id: int):
        await self._conn.execute("DELETE FROM sects WHERE id = ?", (sect_id,))
        await self._conn.commit()

    async def get_sect_by_name(self, sect_name: str) -> Optional[Dict[str, Any]]:
        async with self._conn.execute(
            "SELECT * FROM sects WHERE name = ?", (sect_name,)
        ) as cursor:
            row = await cursor.fetchone()
    ...
    async def get_sect_members(self, sect_id: int) -> List[Player]:
        async with self._conn.execute(
            "SELECT * FROM players WHERE sect_id = ?", (sect_id,)
        ) as cursor:
            return [Player(**dict(row)) for row in await cursor.fetchall()]

    async def update_player_sect(
        self, user_id: str, sect_id: Optional[int], sect_name: Optional[str]
    ):
        await self._conn.execute(
            "UPDATE players SET sect_id = ?, sect_name = ? WHERE user_id = ?",
            ...
            for row in rows:
                ...
                if item_info:
                    ...
                    inventory_list.append(
                        {
                            "item_id": item_id,
                            "name": f"未知物品(ID:{item_id})",
                            "quantity": quantity,
                            "description": "此物品信息已丢失",
                            "rank": "未知",
                            "type": "未知",
                        }
                    )
    ...
    async def get_item_from_inventory(
        self, user_id: str, item_id: str
    ) -> Optional[Dict[str, Any]]:
        async with self._conn.execute(
            "SELECT item_id, quantity FROM inventory WHERE user_id = ? AND item_id = ?",
            (user_id, item_id),
        ) as cursor:
            row = await cursor.fetchone()
            return dict(row) if row else None

    async def add_items_to_inventory_in_transaction(
        self, user_id: str, items: Dict[str, int]
    ):
        try:
            await self._conn.execute("BEGIN")
    ...
    async def transactional_buy_item(
        self, user_id: str, item_id: str, quantity: int, total_cost: int
    ) -> Tuple[bool, str]:
        try:
            await self._conn.execute("BEGIN")
            cursor = await self._conn.execute(
    ...
    async def transactional_apply_item_effect(
        self, user_id: str, item_id: str, quantity: int, effect: PlayerEffect
    ) -> bool:
        try:
            await self._conn.execute("BEGIN")
            cursor = await self._conn.execute(
